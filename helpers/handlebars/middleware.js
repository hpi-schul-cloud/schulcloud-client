...
const makeActive = (items, currentUrl) => {
	...
	if (currentUrl.split("/")[1] === "courses") {
		const coursesSidebarItems = items.filter(
			(i) => i.link === "/rooms-overview/"
		);
	...
	return items.map((item) => {
		...
		if (item.children && item.childActive) {
			...
			if (
				item.children.filter((child) => child.class == "active")
					.length == 0
			) {
				item.class += " active";
			}
...
module.exports = (req, res, next) => {
	...
	// admin views
	const newSchoolAdminPageAsDefault =
	...
	const adminChildItems = [
		...
		},
		{
			...
			testId: "Schule",
			// eslint-disable-next-line max-len
			icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M22 22V11H17V2L2 7V22H22ZM4 8.3736L15 4.91206V11H10V20H4V8.3736ZM15 20H12V13H20V20H17V17H15V20Z"/></svg>',
			isExternalIcon: true,
	...
	Promise.all([notificationsPromise]).then(([notifications]) => {
		res.locals.notifications = (notifications.data || []).map(
			(notification) => {
