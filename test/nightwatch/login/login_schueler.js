...
module.exports = {
	...
	'Schul-Cloud Login': function login(browser) {
		browser
			.setValue('input[name=username]', studentName)
	...
	'Schul-Cloud Visit Every Subpage': function visitSubpages(browser) {
		// News
		...
		browser.expect
			.element('#titlebar h1#page-title')
			.text.to.contain('Gestellte Aufgaben')
			.before(10000);

		browser.url(`${baseUrl}homework/private/`);
		browser.expect
			.element('#titlebar h1#page-title')
			.text.to.contain('Meine ToDos')
			.before(10000);

		browser.url(`${baseUrl}homework/archive/`);
		browser.expect
			.element('#titlebar h1#page-title')
			.text.to.contain('Archivierte Aufgaben und ToDos')
			.before(10000);
