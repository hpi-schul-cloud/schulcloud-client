...
module.exports = {
	...
	'Schul-Cloud Checkups': function (browser) {
		...
		browser.expect.element('#main-content > section.section-timetable > div.row > div > h5').text
			.to.contain('Stundenplan').before(10000);
		browser.setCookie({
			name: 'releaseDate',
	...
	'Schul-Cloud Visit Every Subpage': function (browser) {
		...
		browser.waitForElementVisible('#titlebar h4#page-title', 10000);
		...
		browser.expect.element('#titlebar h4#page-title').text.to.contain('Lern-Store').before(10000);
