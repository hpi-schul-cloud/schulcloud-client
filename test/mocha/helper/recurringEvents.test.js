...
describe('Recurring Event Helper tests', () => {
	...
	describe('createRecurringEvents', () => {
		...
		it('check edge cases', () => {
			...
			const untilTime = new Date('2020-12-25T10:00:00.000Z').getTime();
			const recEventStartAtTheFirstCourseDay = {
				type: 'event',
				included: [
					{
						type: 'rrule',
						...
						attributes: {
							freq: 'WEEKLY',
							until: '2020-12-25T00:00:00.000Z',
							wkst: 'FR',
						},
					},
				],
			...
			const recurringEvents = recurringEventsHelper.createRecurringEvents(recEventStartAtTheFirstCourseDay);
			...
			expect(timesHelper.dateToDateTimeString(recurringEvents[2].start)).to.equal('25.12.2020 11:00');
		});
	});
});
