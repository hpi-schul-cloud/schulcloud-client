    ...
    .then(([course, lessons, homeworks, courseGroups]) => {
      ...
      courseGroups = permissionHelper.userHasPermission(
        ...
        : (courseGroups.data || []).filter(cg =>
            cg.userIds.some(user => user._id === res.locals.currentUser._id)
          );
      ...
      res.render(
        ...
        Object.assign({}, course, {
          title: course.name,
          lessons,
          homeworks: homeworks.filter(task => {
            return !task.private;
          }),
          myhomeworks: homeworks.filter(task => {
            return task.private;
          }),
          ltiToolIds,
          courseGroups,
...
router.get("/share/:id", (req, res, next) =>
  api(req)
    ...
    .catch(() =>
      res.json({ msg: "ShareToken is not in use.", status: "error" })
    )
);
