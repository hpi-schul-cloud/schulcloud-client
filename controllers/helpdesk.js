...
const returnAdminPrefix = (roles, res) => {
	...
	roles.map((role) => {
		...
		role.name === 'teacher'
			...
			: (prefix = res.$t(
				'administration.controller.headline.administration',
			));
	});
	return prefix;
};
	...
	if (state === 'submitted' || state === 'closed') {
		...
		actions.push(
			...
			},
			{
				...
				method: 'post',
				title: res.$t(
					'administration.controller.link.sendEntryToDevelopmentTeam',
				),
			},
		);
	}
...
router.post(
	...
	(req, res, next) => {
		if (!req.body.subject && req.body.target) {
			if (req.body.target === 'HPI') {
				// Contact Admin
				// title? Y: Feedback N: Problem
				req.body.subject =					req.body.type
		...
		files.forEach((element) => {
			if (
				...
				throw new Error(
					res.$t('global.text.fileWrongFormat', {
		...
		api(req)
			...
			.then(() => {
				req.session.notification = {
					type: 'success',
					message: res.$t('helpdesk.text.feedbackSuccessful'),
				};
				redirectHelper.safeBackRedirect(req, res);
			})
			.catch((err) => {
		...
		api(req)
			...
			.then((data) => {
				...
				res.render('helpdesk/overview', {
					title: res.$t(
						'administration.controller.headline.helpdesk',
						{
							title,
						},
					),
