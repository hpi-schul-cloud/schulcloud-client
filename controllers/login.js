...
Handlebars.registerHelper("oauthLink", (id) => {
	const apiUrl = `${Configuration.get(
		"PUBLIC_BACKEND_URL"
	)}/v3/sso/login/${id}`;
	return apiUrl;
});
...
router.post("/login/", (req, res, next) => {
	const { username, password, system, schoolId, redirect } = req.body;
...
router.get("/login/email", (req, res) => {
	...
	if (req.query.redirect) {
		queryString.append(
			"redirect",
			redirectHelper.getValidRedirect(req.query.redirect)
		);
	}

	const redirect = redirectHelper.joinPathWithQuery(
		"/login",
...
router.post("/login/ldap", async (req, res) => {
	const { username, password, schoolId, system, redirect } = req.body;
...
// eslint-disable-next-line consistent-return
const redirectOAuth2Authentication = async (
	req,
	res,
	systemId,
	migration,
	...
	let system;
	try {
		system = await api(req, { version: "v3" }).get(
			`/systems/public/${systemId}`
		);
	} catch (error) {
	...
	const authenticationUrl = authHelper.getAuthenticationUrl(
		oauthConfig,
		state
	);
...
router.post("/login/oauth2", async (req, res) => {
	const { systemId, migration, redirect } = req.body;
...
router.get("/login/oauth2/:systemId", async (req, res) => {
	...
	const { migration, redirect } = req.query;
...
const filterSchoolsWithLdapLogin = (schools) =>
	schools
		// eslint-disable-next-line max-len
		.filter((school) =>
			school.systems.some(
				(system) => system.type === "ldap" && !system.oauthConfig
			)
		);
...
async function getOauthSystems(req) {
	return api(req, { version: "v3" })
		...
		.catch((err) =>
			logger.error("error loading oauth system list", formatError(err))
		);
}
...
const renderLogin = async (req, res) => {
	...
	if (req.query.error) {
		...
		if (
			req.query.provider === "iserv" &&
			req.query.error !== "sso_oauth_access_denied"
		) {
...
router.all("/login/superhero/", async (req, res, next) => {
	...
	await renderLogin(req, res).catch(next);
});
...
router.get("/login/success", authHelper.authChecker, async (req, res) => {
	if (res.locals.currentUser) {
		...
		const { haveBeenUpdated, consentStatus } = await api(req).get(
			`/consents/${user._id}/check/`,
			{
				qs: {
					simple: true,
				},
	...
	if (accountId && systemId && schoolId) {
		const schools = await LoginSchoolsCache.get(req);
		if (schools.length > 0) {
			const checkSchool = schools.find(
