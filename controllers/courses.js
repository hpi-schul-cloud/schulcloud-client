...
const editCourseHandler = (req, res, next) => {
	...
	Promise.all([
		...
		studentsPromise,
		subjectsPromise
	]).then(([course, classes, teachers, students, subjects]) => {
		...
		res.render("courses/edit-course", {
			...
			submitLabel: req.params.courseId
			...
			subjects: markSelected(subjects, [course.subjectId]),
	...
	Promise.all([
		...
		substitutionCourses = substitutionCourses.data.map(course => {
			...
			(course.times || []).forEach(time => {
				...
				time.weekday = recurringEventsHelper.getWeekdayForNumber(
					time.weekday
				);
				course.secondaryTitle += `<div>${time.weekday} ${
					time.startTime
...
router.get("/:courseId", function(req, res, next) {
	Promise.all([
		...
		.then(([course, lessons, homeworks, courseGroups]) => {
			let ltiToolIds = (course.ltiToolIds || []).filter(
...
router.get("/:courseId/addStudent", function(req, res, next) {
	...
	api(req)
		.get("/courses/" + req.params.courseId)
		.then(course => {
...
router.post("/:courseId/importTopic", function(req, res, next) {
	...
	api(req)
		.get("/lessons/", {
			qs: { shareToken: shareToken, $populate: ["courseId"] }
		})
		.then(lessons => {
