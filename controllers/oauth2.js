...
router.get('/login/success', csrfProtection, auth.authChecker, (req, res, next) => {
	...
	return api(req, { version: 'v3' }).patch(`/oauth2/loginRequest/${req.session.login_challenge}/?accept=1`,
...
router.all('/logout', csrfProtection, auth.authChecker, (req) => api(req, { version: 'v3' }).get('/oauth2/logoutRequest'));
...
router.all('/logout/redirect', csrfProtection, auth.authChecker, (req, res, next) => {
	...
	return api(req, { version: 'v3' }).patch(`/oauth2/logoutRequest/${req.query.logout_challenge}`, { body })
...
const acceptConsent = (r, w, challenge, grantScopes, remember = false) => {
	...
	return api(r, { version: 'v3' }).patch(`/oauth2/consentRequest/${challenge}/?accept=1`, { body })
...
router.get('/consent', csrfProtection, auth.authChecker, (r, w, next) => {
	...
	return api(r, { version: 'v3' }).get(`/oauth2/consentRequest/${r.query.consent_challenge}`)
