...
const handleTeamSubmissionsBody = (body, currentUser) => {
	...
	body.teamSubmissionOptions === "courseGroup"
		? (body.teamMembers = [currentUser._id])
...
function collectUngradedFiles(submissions) {
	const ungradedSubmissionsWithFiles = submissions.filter(
		(submission) => !submission.submission.graded
	);
	const ungradedFiles = ungradedSubmissionsWithFiles.flatMap(
		(submission) => submission.submission.submissionFiles.filesStorage.files
	);

	return {
		empty: _.isEmpty(ungradedFiles),
		urls: ungradedFiles
			.map(filesStoragesHelper.getFileDownloadPath)
			.join(" "),
...
const prepareRequestBodyForHomework = (service, req, res) => {
	if (service === "homework") {
		...
		if (!courseId || (courseId && courseId.length <= 2)) {
			// todo for what is lessonId.length <= 2 ?
			req.body.courseId = null;
			req.body.private = true;
		}
		if (!lessonId || (lessonId && lessonId.length <= 2)) {
			// todo for what is lessonId.length <= 2 ?
		...
		if (req.body.dueDate && req.body.availableDate >= req.body.dueDate) {
			req.session.notification = {
				type: "danger",
				message: res.$t(
					"homework._task.text.startDateBeforeSubmissionDate"
				),
			};
				...
				api(req)
					...
					.then((course) => {
						sendNotification(
							data.courseId,
							res.$t(
								"homework._task.text.newHomeworkCourseNotification",
								{ coursename: course.name }
							),
							res.$t(
								"homework._task.text.newHomeworkDueDateNotification",
								{
				...
				api(req)
					...
					.then((homework) => {
						sendNotification(
							data.studentId,
							res.$t(
								"homework._task.text.submissionGradedNotification",
								{
...
const getUpdateHandler = (service) =>
	function updateHandler(req, res, next) {
		...
		if (service === "homework") {
			...
			if (req.body.archive) {
				return api(req)
					.get(`/homework/${req.params.id}`, {})
					.then((homework) => {
						const archived = homework.archived || [];
						if (
							archived.includes(res.locals.currentUser._id) &&
							req.body.archive === "open"
						) {
							archived.splice(
								homework.archived.indexOf(
			...
			if (
				!req.body.courseId ||
				(req.body.courseId && req.body.courseId.length <= 2)
			) {
				req.body.courseId = null;
				req.body.private = true;
			}
			if (
				!req.body.lessonId ||
				(req.body.lessonId && req.body.lessonId.length <= 2)
			) {
				req.body.lessonId = null;
			}
			...
			if (
				...
				req.body.dueDate &&
				req.body.availableDate >= req.body.dueDate
			) {
				req.session.notification = {
					type: "danger",
					message: res.$t(
...
router.get("/new", (req, res, next) => {
	const coursesPromise = getSelectOptions(
		req,
		`users/${res.locals.currentUser._id}/courses`,
		{
			$limit: false,
		}
	);
	Promise.resolve(coursesPromise).then(async (courses) => {
		...
		const taskFilesStorageData = await filesStoragesHelper.filesStorageInit(
			schoolId,
			undefined,
			parentType,
			false,
...
router.get("/:assignmentId/edit", (req, res, next) => {
	api(req)
		.get(`/homework/${req.params.assignmentId}`, {
			qs: {
				$populate: ["courseId"],
			},
		})
		.then((assignment) => {
			const isTeacher =
				...
				((assignment.courseId || {}).teacherIds || []).includes(
					res.locals.currentUser._id
				);
			const isSubstitution = (
				(assignment.courseId || {}).substitutionIds || []
			).includes(res.locals.currentUser._id);
			...
			if (assignment.availableDate) {
				assignment.availableDate = timesHelper.fromUTC(
					assignment.availableDate
				);
			} else {
				assignment.availableDate = timesHelper
					.currentDate()
					.toISOString();
			}
			...
			Promise.resolve(coursesPromise)
				.then(async (courses) => {
					courses.sort((a, b) =>
						a.name.toUpperCase() < b.name.toUpperCase() ? -1 : 1
					);
					...
					const parentType = "tasks";
					const taskFilesStorageData =
						await filesStoragesHelper.filesStorageInit(
							schoolId,
							parentId,
							parentType,
							false,
					...
					if (assignment.courseId?._id) {
						const lessonsPromise = getSelectOptions(
							req,
							"lessons",
							{
						...
						Promise.resolve(lessonsPromise).then((lessons) => {
							...
							res.render("homework/edit", {
								title: res.$t(
									"homework._task.headline.editTask"
								),
								submitLabel: res.$t("global.button.save"),
								closeLabel: res.$t("global.button.cancel"),
								method: "patch",
...
router.get("/:assignmentId", (req, res, next) => {
	api(req)
		...
		.then(async (assignment) => {
			...
			if (assignment.dueDate) {
				assignment.dueDate = timesHelper.fromUTC(assignment.dueDate);
				const dueDateTimeStamp = timesHelper.splitDate(
					assignment.dueDate,
					res.$t("format.date")
				).timestamp;
			...
			async function findSubmissionFiles(
				submission,
				submitters,
				teachers,
				readonly
				...
				if (submission.submitted || isCreator) {
					const result = await filesStoragesHelper.filesStorageInit(
						schoolId,
						parentId,
						parentType,
						readonly,
				...
				submissionFilesStorageData.files = filesStorage.files.filter(
					(file) => submitters.has(file.creatorId)
				);
				submissionFilesStorageData.readonly =
					readonly || (!isCreator && isTeacher);

				const gradeFilesStorageData = _.clone(filesStorage);
				gradeFilesStorageData.files = filesStorage.files.filter(
					(file) => teachers.has(file.creatorId)
				);
				gradeFilesStorageData.readonly = !isTeacher;

				submission.submissionFiles = {
					filesStorage: submissionFilesStorageData,
				};
			...
			await Promise.all(promises)
				.then(async ([submissions, course, courseGroups]) => {
					assignment.submission = _.cloneDeep(
						(submissions || {}).data
							.map((submission) => {
								submission.teamMemberIds = (
									submission.teamMembers || []
								).map((e) => e._id);
								submission.courseGroupMemberIds =
									(submission.courseGroupId || {}).userIds ||
									[];
								submission.courseGroupMembers = (
									_.find(
										(courseGroups || {}).data,
										(cg) =>
											JSON.stringify(cg._id) ===
											JSON.stringify(
												(submission.courseGroupId || {})
													._id
											)
									) || {}
								).userIds; // need full user objects here, double populating not possible above
								submission.submitters = new Set([
							...
							.filter(
								(submission) =>
									(submission.studentId || {})._id ==
										res.locals.currentUser._id ||
									submission.teamMemberIds.includes(
					...
					courseGroups = permissionHelper.userHasPermission(
						...
						: ((courseGroups || {}).data || [])
								...
								.filter((cg) =>
									assignment.maxTeamMembers
										? cg.userIds.length <=
										  assignment.maxTeamMembers
										: true
								); // filter to big courseGroups

					const courseGroupSelected = (
						(assignment.submission || {}).courseGroupId || {}
					)._id;
					...
					(assignmentCourse.teacherIds || []).forEach((teacher) =>
						teachers.add(teacher)
					);
					(assignmentCourse.substitutionIds || []).forEach(
					...
					if (
						!assignment.private &&
						(isTeacher || assignment.publicSubmissions)
					) {
						// Daten für Abgabenübersicht
						const sortByStudentAttribute = (attr) => (a, b) =>
							a.studentId[attr].toUpperCase() <
							b.studentId[attr].toUpperCase()
								? -1
								: 1;
						...
						const studentSubmissions = students.map((student) => ({
							...
							submission: assignment.submissions.filter(
								(submission) =>
									...
									submission.teamMembers?.includes(
						...
						((assignment.courseId || {}).userIds || []).forEach(
							(e) => {
								if (
									!studentsWithSubmission.includes(
										e.toString()
									)
								) {
						...
						assignment.submissions = assignment.submissions.map(
							(s) => ({ submission: s })
						);

						renderOptions.studentSubmissions = studentSubmissions;
						renderOptions.studentsWithoutSubmission =
							studentsWithoutSubmission;

						assignment.submissions = await Promise.all(
							assignment.submissions.map(async (submission) => ({
								submission: await findSubmissionFiles(
									submission.submission,
									submission.submission.submitters,
									teachers,
									true
								),
							}))
						);
						renderOptions.ungradedFileSubmissions =
							collectUngradedFiles(assignment.submissions);
					}

					const submissionFilesStorageData = {
						filesStorage: {
					...
					if (assignment.submission) {
						assignment.submission = await findSubmissionFiles(
							assignment.submission,
							assignment.submission.submitters,
							teachers,
							!assignment.submittable
						);
					}
