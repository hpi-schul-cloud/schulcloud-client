    ...
    .then(eve =>
      Promise.all(
        ...
        return (events || []).map(event => {
          ...
          const eventStartRelativeMinutes =
            (eventStart.getUTCHours() - timeStart) * 60 +
            eventStart.getMinutes();
          const eventEndRelativeMinutes =
            (eventEnd.getUTCHours() - timeStart) * 60 + eventEnd.getMinutes();
          const eventDuration =
            eventEndRelativeMinutes - eventStartRelativeMinutes;
    ...
    .then(([events, assignedHomeworks, news, newestReleases]) => {
      ...
      if (roles.includes("student")) {
        ...
        studentHomeworks = assignedHomeworks.filter(
          homework =>
            (!homework.submissions || homework.submissions === 0) &&
