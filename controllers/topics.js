...
const etherpadBaseUrl =
	process.env.ETHERPAD_BASE_URL ||
	"https://etherpad.schul-cloud.org/etherpad/p/";
	...
	if (req.params.topicId) {
		action =
			...
			(req.query.courseGroup
				? "?courseGroup=" + req.query.courseGroup
				: "");
		method = "patch";
...
router.post("/:id/share", function(req, res, next) {
	...
	api(req)
		.get("/lessons/" + req.params.id)
		.then(topic => {
			topic.shareToken = topic.shareToken || shortId.generate();
			api(req)
				.patch("/lessons/" + req.params.id, { json: topic })
	...
	Promise.all([
		...
		res.render(
			...
			Object.assign({}, lesson, {
				title: lesson.name,
				homeworks: homeworks.filter(function(task) {
					return !task.private;
				}),
				myhomeworks: homeworks.filter(function(task) {
					return task.private;
				}),
				courseId: req.params.courseId,
...
router.patch("/:topicId", async function(req, res, next) {
	...
	api(req)
		.patch("/lessons/" + req.params.topicId, {
			json: data // TODO: sanitize
		})
		.then(_ => {
			if (req.query.json) {
...
router.delete("/:topicId", function(req, res, next) {
	api(req)
		.delete("/lessons/" + req.params.topicId)
		.then(_ => {
			res.sendStatus(200);
		})
		.catch(err => {
...
router.delete("/:topicId/materials/:materialId", function(req, res, next) {
	api(req)
		...
		.then(_ => {
			api(req)
				.delete("/materials/" + req.params.materialId)
				.then(_ => {
					res.sendStatus(200);
				});
		});
...
async function createNewNexBoards(req, res, contents = []) {
	return await Promise.all(
		contents.map(async content => {
			if (
				content.component === "neXboard" &&
				content.content.board === "0"
			) {
				try {
					const board = await getNexBoardAPI().createBoard(
						content.content.title,
						content.content.description,
						await getNexBoardProjectFromUser(
							req,
...
const getNexBoardProjectFromUser = async (req, user) => {
	...
	if (typeof preferences.nexBoardProjectID === "undefined") {
		const project = await getNexBoardAPI().createProject(
			user._id,
			user._id
		);
