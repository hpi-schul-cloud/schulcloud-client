...
const cutEditOffUrl = url => {
  //nicht optimal, aber req.header('Referer') gibt auf einer edit Seite die edit Seite, deshalb diese URL Manipulation
  let workingURL = url;
      ...
      return api(req)
        ...
        .catch(err => {
          req.session.notification = {
            type: "danger",
            message: `Fehler beim Erstellen des Registrierungslinks. Bitte selbstständig Registrierungslink im Nutzerprofil generieren und weitergeben. ${(
              err.error || {}
            ).message ||
              err.message ||
              err ||
...
${res.locals.theme.short_title}-Team`
      ...
      .catch(err => {
        ...
        req.session.notification = {
          type: "danger",
          message: `Nutzer erstellt. Fehler beim Versenden der E-Mail. Bitte selbstständig Registrierungslink im Nutzerprofil generieren und weitergeben. ${(
            err.error || {}
          ).message ||
            err.message ||
            err ||
  ...
  return async function(req, res, next) {
    try {
      ...
      if (stats.success) {
        req.session.notification = {
          type: "success",
          message: `${stats.users.successful} von ${numberOfUsers} Nutzer${
            numberOfUsers > 1 ? "n" : ""
          } importiert.`
        };
      } else {
...
const getTeacherUpdateHandler = () => {
  return async function(req, res, next) {
    ...
    const addedClasses = (req.body.classes || []).filter(function(i) {
      return !usersClasses.includes(i);
    });
    const removedClasses = usersClasses.filter(function(i) {
      return !(req.body.classes || []).includes(i);
    });
    ...
    api(req)
      ...
      .then(currentClass => {
        ...
        Promise.all([
          ...
          const teacherIds = currentClass.teacherIds.map(t => {
            return t._id;
          });
          teachers.forEach(t => {
          ...
          const studentIds = currentClass.userIds.map(t => {
            return t._id;
          });
          students.forEach(s => {
            ...
            notes: [
              {
                ...
                content: `Gib den Registrierungslink zunächst an die Eltern weiter. Diese legen die Schülerdaten an und erklären elektronisch ihr Einverständnis. Der Schüler ist dann in der ${
                  res.locals.theme.short_title
                } registriert und du siehst ihn in deiner Klassenliste. Der Schüler kann sich mit seiner E-Mail-Adresse und dem individuellen Initial-Passwort einloggen. Nach dem ersten Login muss jeder Schüler sein Passwort ändern. Ist der Schüler über 14 Jahre alt, muss er zusätzlich selbst elektronisch sein Einverständnis erklären, damit er die ${
                  res.locals.theme.short_title
                } nutzen kann.`
              },
              {
                ...
                content:
                  "Gib den Registrierungslink direkt an den Schüler weiter. Die Schritte für die Eltern entfallen automatisch."
              },
...
getTeamFlags = team => {
  const createdAtOwnSchool =
    '<i class="fa fa-building-o team-flags" data-toggle="tooltip" data-placement="top" title="An eigener Schule gegründetes Team"></i>';
  const hasMembersOfOtherSchools =
    '<i class="fa fa-bus team-flags" data-toggle="tooltip" data-placement="top" title="Beinhaltet Schul-externe Mitglieder"></i>';
  const hasOwner =
    '<i class="fa fa-briefcase team-flags" data-toggle="tooltip" data-placement="top" title="Team hat Eigentümer"></i>';
...
getTeamMembersButton = counter => {
  return `<div class="btn-show-members" role="button">${counter}<i class="fa fa-user team-flags"></i></div>`;
};

getTeamSchoolsButton = counter => {
  return `<div class="btn-show-schools" role="button">${counter}<i class="fa fa-building team-flags"></i></div>`;
};
