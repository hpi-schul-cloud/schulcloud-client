...
//copy fonts
gulp.task("fonts", () => {
	return beginPipe("./static/fonts/**/*.*").pipe(
		gulp.dest(`./build/${themeName()}/fonts`)
	);
});
...
gulp.task("vendor-optimized-assets", () => {
	return beginPipe(["./static/vendor-optimized/**/*.*"]).pipe(
		gulp.dest(`./build/${themeName()}/vendor-optimized`)
	);
});
...
gulp.task("node-modules", () =>
	Promise.all(
		nodeModules.map(module =>
			beginPipe([`./node_modules/${module}/**/*.*`]).pipe(
				gulp.dest(`./build/${themeName()}/vendor-optimized/${module}`)
			)
		)
	)
);
...
gulp.task("sw-workbox", () => {
	return beginPipe(["./static/scripts/sw/workbox/*.js"]).pipe(
		gulp.dest(`./build/${themeName()}/scripts/sw/workbox`)
	);
});
