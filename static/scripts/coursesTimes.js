...
export const populateCourseTimes = function populateCourseTimes(modal, courseTimes) {
	const $courseTimes = modal.find('.course-times');
	// cleanup
	$courseTimes.find('tr:gt(0)').not('.new-course-time-template').remove();

	// add new field
	courseTimes.forEach((time) => {
		const $courseTime = addNewCourseTime($courseTimes);
		populateCourseTime($courseTime, time);
	});
};

const deleteCourseTime = function (courseTimeId) {
	$(`#${courseTimeId}`).remove();
};

/** fix the bootstrap-chosen solution * */
// eslint-disable-next-line func-names
const fixChosen = function (selectElement) {
...
let addNewCourseTime = function addNewCourseTime(div) {
	const $courseTimesTable = div.find('.add-course-times');
	const $newCourseTime = div.find('.new-course-time-template').clone();
	const _id = guidGenerator();

	const logic = function (currentDateTime) {
...
$(document).ready(() => {
	$('.new-course-time-add').click(function (e) {
		...
		const $timesContainer = $($(this).attr('data-timesref'));
