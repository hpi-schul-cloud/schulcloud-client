		...
		return (
			<div
				className={`content-block ${
					this.props.hidden ? "content-block-hidden" : ""
				}`}
			>
				<div className="card">
					<div className="card-header">
						<div className="input-group">
							<span className="input-group-btn">
								<i className="fa fa-arrows move-handle" />
								<a
									className="btn btn-secondary hidden-toggle"
									...
									title={
										this.props.hidden
											? $t(
													"topic.topicEdit.label.openSection"
											  )
											: $t(
													"topic.topicEdit.label.lockSection"
											  )
									}
									data-original-title={
										this.props.hidden
							...
							<input
								placeholder={$t(
									"topic.topicEdit.input.sectionTitle"
								)}
							...
							<div className="input-group-btn">
								...
								<div className="dropdown-menu dropdown-menu-right">
									<a
										...
										onClick={this.onRemoveWithCallback.bind(
											this
										)}
									>
										<span>
					...
					<div className="card-block">
						<this.props.type
							{...this.props}
							addOnBeforeRemoveCallback={this.addOnBeforeRemoveCallback.bind(
								this
							)}
						/>
					</div>
...
class TopicBlockList extends React.Component {
	 ...
	 */
	render() {
		const neXboardEnabled =
			$contentBlocksContainer.data("nexboardenabled") === true;
		return (
			<div>
				<SortableList
				...
				<div className="form-group">
					...
					>
						...
						>{`+ ${$t("topic.topicEdit.button.text")}`}</button>
						<button
							type="button"
							className="btn btn-secondary"
							onClick={this.addBlock.bind(this, TopicGeoGebra)}
						>{`+ ${$t(
							"topic.topicEdit.button.geoGebraWorksheet"
						)}`}</button>
						<button
							type="button"
							className="btn btn-secondary"
							onClick={this.addBlock.bind(this, TopicResources)}
						>{`+ ${$t("topic.topicEdit.button.material")}`}</button>
						{neXboardEnabled ? (
							<button
								type="button"
								className="btn btn-secondary"
								onClick={this.addBlock.bind(
									this,
									TopicNexboard
								)}
							>
						...
						>{`+ ${$t("topic.topicEdit.button.task")}`}</button>
					</div>
				</div>
			</div>
		);
			...
			<div className="card">
				...
				<div className="card-footer">
					<small className="text-muted">
						via {(this.props.resource || {}).client}
					</small>
					<a
...
class TopicResources extends TopicBlock {
	...
	addResource(resource = "") {
		...
		if (!resource) {
			const isCourseGroupTopic =
				$contentBlocksContainer.data("iscoursegroup") !== undefined;
			// open content search popup
			const resourcePopup = window.open(
				`/content/?inline=1&isCourseGroupTopic=${isCourseGroupTopic}`,
				"content-search",
			...
			<div className="input-group">
				<span className="input-group-btn">
					<a
						...
						data-toggle="tooltip"
						data-placement="top"
						title={$t("topic.topicEdit.label.youllFindTheIdOn")}
					>
				...
				<div className="input-group">
					<span className="input-group-btn">
						<a
							...
							data-placement="top"
							title={$t(
								"topic.topicEdit.label.theLinkHasToBeginWith",
			...
			<div>
				<div type="hidden" className="form-group">
					...
					<input
						...
						type="text"
						placeholder={$t(
							"topic.topicEdit.input.brainstormAboutXYZ"
						)}
				...
				<div className="form-group">
					...
					<textarea
						...
						placeholder={$t(
							"topic.topicEdit.input.createsListInEtherpad"
						)}
					>
...
class TopicNexboard extends TopicBlock {
	...
	handleChange() {
		const id = $(`select[id=${this.state.id}]`)
	...
	render() {
		return (
			<div>
				...
				<div className="form-group">
					<label>
						{$t("topic.topicEdit.label.descriptionNeXboard")}
					</label>
					<textarea
						className="form-control"
						name={`contents[${this.props.position}][content][description]`}
						placeholder={$t(
							"topic.topicEdit.input.createsListInNeXboard"
						)}
					>
						{(this.props.content || {}).description}
					</textarea>
				...
				<div className="form-group">
					...
					<select
						name={`contents[${this.props.position}][content][board]`}
						className="chosen-select"
						data-placeholder={$t(
							"topic.topicEdit.input.selectNeXboard"
						)}
						id={this.state.id}
						value={(this.props.content || {}).board}
					>
						{(this.props.content || {}).board ? (
							<option value={this.props.content.board}>
								{$t("topic.topicEdit.input.keepNeXboard")}
							</option>
						) : (
							""
						)}
						<option value={this.state.newBoard}>
							{$t("topic.topicEdit.input.createNewNeXboard")}
						</option>
					</select>
				</div>
				<input
...
ReactDOM.render(<TopicBlockList />, $contentBlocksContainer[0]);
