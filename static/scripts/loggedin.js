...
function toggleSidebarItemGroup(groupData) {
	...
	const toggleIcon = document.querySelector(
		`#${groupData.groupName}-toggle-icon`
	);
	if (toggleIcon) {
...
function toggleMobileSearch() {
	document
		.querySelector(".search-wrapper .input-group")
		.classList.toggle("active");
	document
		.querySelector(".search-wrapper .mobile-search-toggle .fa")
...
$(document).ready(function () {
	...
	const sidebarToggleButtonInTopbar = document.querySelector(
	...
	const sidebarToggleButtonInSidebar = document.querySelector(
	...
	if (feedbackSelector) {
		feedbackSelector.onchange = function () {
			if (feedbackSelector.value === "problem") {
				...
				document
					.querySelectorAll(
						"#problemDiv input, #problemDiv textarea, #problemDiv select"
					)
					.forEach((node) => {
						node.required = true;
					});
				document
					.querySelectorAll(
						"#userstoryDiv input, #userstoryDiv textarea, #userstoryDiv select"
					)
					.forEach((node) => {
						node.required = false;
					});
...
document.querySelectorAll("#main-content a").forEach((a) => {
	...
	if (a.querySelector("img, .fa") == null && href) {
		if (
			!(
				href.startsWith("https://dbildungscloud.de") ||
				href.startsWith("#") ||
				href.startsWith("/") ||
				href === ""
			)
