$(document).ready(function () {
    $('.btn-member').on('click', function (e) {
        ...
        $.ajax({
            ...
            if(res.course.members.length != 0) {
                courseMembers = '<ol>';
                res.course.members.forEach(member => {
                    courseMembers = courseMembers + '<li>' + member.fullName + '</li>';
                });
                courseMembers = courseMembers + '</ol>';
            }

            populateModal($memberModal, '.modal-title', 'Teilnehmer des Kurses: '.concat(res.course.name));
