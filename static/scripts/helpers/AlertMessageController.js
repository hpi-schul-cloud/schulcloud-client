...
class AlertMessageController {
	...
	buildMessage(message) {
		...
		if (this.loggedin) {
			...
			${message.text}
			<br>
			<div class="alert-date text-nowrap text-muted-alert" style="float: left;">Updated: ${datetime.fromNow(message.timestamp)}</div>
			<div class="alert-date text-nowrap text-muted-alert" style="float: left;"> <span>|</span> Created: ${datetime.toDateTimeString(message.created_at)}</div>
			...
			${message.text}
			<br>
			<div class="text-muted-alert" style="float: left;"> Updated: ${datetime.toDateTimeString(message.timestamp)}</div>
			<div class="text-muted-alert" style="float: left;"> <span>|</span> Created: ${datetime.toDateTimeString(message.created_at)}</div>
			<div style="clear: both;"></div>`;
		}
		return item;
	}
